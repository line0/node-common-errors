...
// uses the V8-specific method to capture a stack trace if available, otherwise falls back to a generic solution
module.exports = function captureStackTrace(error, errorClass = error.constructor) {
  if (isCaptureStackTraceSupported) {
    Error.captureStackTrace(error, errorClass);
  } else Object.defineProperty(error, 'stack', {
    value: Error().stack,
    writable: true,
    configurable: true
  });
