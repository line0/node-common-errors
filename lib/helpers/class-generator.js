...
module.exports = function generateErrorClass(name, options){
 ...
 var classGeneratorFnNew = (classConstructor) => {
   return function (...args) {
     var instance = Object.create({
       prototype: Object.create(options.extends.prototype),
       length: options.args.length,
       name,
       generateMessage: options.generateMessage,
       captureStackTrace: function captureStackTrace(error, error_class) {
         Error.captureStackTrace(error, error_class);
         if (error.inner_error && error.inner_error.stack)
           error.stack += '\n--- inner error ---\n' + error.inner_error.stack;
       },
     });
     classConstructor.apply(instance, args);
     return instance;
   };
 };

 var Class = classGeneratorFnNew(classConstructor);
